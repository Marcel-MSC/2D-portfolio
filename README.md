...
An interactive, game-like personal portfolio built as a top-down 2D scene in the style of classic Pokémon / Zelda games. Walk around a pixel-art bedroom and click on objects to learn about me.
...
| Assets / tiles          | **Tiled-style JSON maps** + **spritesheet**               | Map boundaries, spawn points and named collision objects are defined in `map.json` and consumed at runtime. The character and props come from a single sprite atlas (`spritesheet.png`) sliced into a 39×31 grid. |
...
├── constants.js     # Scale factor + dialogue texts for the bedroom.
...
└── scenes/
    └── main.js      # Bedroom scene: loads map.json, wires up boundaries → dialogues.
public/
├── map.png, map.json            # Bedroom tilemap + collision/spawn data.
├── spritesheet.png              # Player + props sprites (39x31 grid).
└── console-controller.svg       # Favicon.
```

Key ideas:

- The scene **fetches its map JSON at runtime**, iterates over layers, and creates Kaboom entities for `boundaries` (static collision rectangles, optionally named) and `spawnpoints`.
- Named boundaries (`pc`, `tv`, `bed`, `resume`, `library`, …) drive the interaction logic — colliding with them opens a dialogue via `displayDialogue` in `utils.js`.
